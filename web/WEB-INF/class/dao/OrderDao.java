...
public class OrderDao {
    ...
    public String getMail(int id) throws SQLException {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
        String sql = "SELECT email FROM `user` WHERE id = (SELECT user_id FROM `order` WHERE id = ?)";
        return r.query(sql, new ScalarHandler<String>(), id);  // 使用 ScalarHandler<String>，并传递 id 参数
    }
